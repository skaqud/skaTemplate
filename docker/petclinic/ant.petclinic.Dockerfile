...
# run 
# 0) docker image build -t test:0.1 ./ant.petclinic.Dockerfile
# 1) docker container run --name petdb -e MYSQL_ROOT_PASSWORD=petclinic -e MYSQL_DATABASE=petclinic  -d mysql
# 2) docker container run -d -p 8080:9966 --link petdb:db --name pet test:0.1
